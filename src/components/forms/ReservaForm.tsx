  ...
  const barbeirosDoServico = useMemo(() => {
    ...
    return barbers.filter((barbeiro) =>
      barbeiro.barber_services?.some(
        (vinculo) => vinculo.service_id === serviceId && vinculo.is_active,
      ),
    );
  }, [barbers, serviceId]);
  ...
  if (services.length === 0 || barbers.length === 0) {
    return (
      <div className="rounded-2xl sm:rounded-3xl border border-yellow-500/30 bg-yellow-500/10 p-4 sm:p-5 text-sm leading-6 text-yellow-100">
        Ainda não existem serviços ou barbeiros ativos para reserva. Cadastre e
        ative pelo painel administrativo.
      </div>
    );
  }

  const semBarbeiroParaServico =
    Boolean(serviceId) && barbeirosDoServico.length === 0;

  return (
    <ActionForm action={asFormAction(criarAgendamento)} className="grid gap-4">
      <input type="hidden" name="slug" value={slug} />
      <div className="rounded-2xl sm:rounded-3xl border border-white/10 bg-black/18 p-4 sm:p-5">
        <div className="mb-4 flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-500/15 text-brand-100">
      ...
      <div className="rounded-2xl sm:rounded-3xl border border-white/10 bg-black/18 p-4 sm:p-5">
        ...
        {semBarbeiroParaServico ? (
          <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-4 text-sm leading-6 text-yellow-100">
            Nenhum barbeiro ativo está vinculado a este serviço. No painel
            administrativo, edite um barbeiro e marque este serviço na lista de
            serviços realizados.
          </div>
        ) : (
          <Select
