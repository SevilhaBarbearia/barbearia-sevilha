...
export function BarbeiroAdminForm({
  barbeiro,
  servicos,
  horarioPadrao,
  slug = "sevilha",
}: BarbeiroAdminFormProps) {
  ...
  return (
    <ActionForm action={action} className="grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <Label htmlFor={`name-${barbeiro?.id ?? "novo"}`}>
            Nome do barbeiro
          </Label>
          <Input
        ...
        <div>
          <Label htmlFor={`phone-${barbeiro?.id ?? "novo"}`}>
            Telefone/WhatsApp
          </Label>
          <Input
      ...
      <div className="rounded-2xl border border-brand-500/20 bg-brand-500/[0.06] p-4 sm:rounded-[1.5rem] sm:p-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-zinc-300">
              Este horário será aplicado internamente de segunda a sábado.
              Domingo fica fechado por padrão.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <div>
            <Label htmlFor={`break-start-${barbeiro?.id ?? "novo"}`}>
              Início do intervalo
            </Label>
            <Input
          ...
          <div>
            <Label htmlFor={`break-end-${barbeiro?.id ?? "novo"}`}>
              Fim do intervalo
            </Label>
            <Input
      ...
      <div>
        ...
        {servicos.length === 0 ? (
          <p className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-3 text-sm text-yellow-100">
            Cadastre pelo menos um serviço antes de vincular serviços ao
            barbeiro.
          </p>
        ) : (
            ...
            {servicos.map((servico) => (
              ...
              >
                <input
                  ...
                  value={servico.id}
                  defaultChecked={
                    !editando || servicosVinculados.has(servico.id)
                  }
      ...
      <BotaoSubmit
        texto={editando ? "Salvar alterações" : "Cadastrar barbeiro"}
      />
    </ActionForm>
  );
}

export function AlternarStatusBarbeiroForm({
  barbeiro,
  slug = "sevilha",
}: {
  barbeiro: Barber;
