...
const campoBase =
  "ui-field w-full rounded-xl border border-white/10 bg-black/25 px-3.5 py-2.5 text-sm font-medium text-white shadow-inner shadow-black/20 outline-none transition placeholder:text-zinc-500 focus:border-brand-500 focus:bg-black/35 focus:ring-4 focus:ring-brand-500/15 disabled:cursor-not-allowed disabled:opacity-60 sm:rounded-2xl sm:px-4 sm:py-3";
...
export function Label({
  ...
  return (
    <label
      className={cn(
        "ui-label mb-1.5 block text-sm font-bold text-zinc-100",
        className,
      )}
      {...props}
    />
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className={cn(
          campoBase,
          "ui-select-trigger group flex min-h-11 cursor-pointer items-center justify-between gap-3 bg-zinc-950/92 pr-3 text-left hover:border-brand-500/70 hover:bg-black/55 disabled:pointer-events-none",
          open &&
            "border-brand-500 bg-black/55 ring-4 ring-brand-500/15",
          className,
        )}
      ...
      {open && (
        <div
          ...
          className="ui-select-menu select-menu-anim absolute left-0 right-0 top-[calc(100%+0.45rem)] z-[80] max-h-72 overflow-auto rounded-2xl border border-brand-500/30 bg-zinc-950/98 p-1.5 shadow-2xl shadow-black/70 ring-1 ring-white/10 backdrop-blur-xl"
        >
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "ui-select-option flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-zinc-200 transition hover:bg-brand-500/15 hover:text-white disabled:cursor-not-allowed disabled:opacity-50",
                  selected &&
                    "ui-select-option-selected bg-brand-500 text-brand-950 hover:bg-brand-500 hover:text-brand-950",
                )}
