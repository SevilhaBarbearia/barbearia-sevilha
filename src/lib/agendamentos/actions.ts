...
import {
  ...
  criarAgendamentoConvidadoSchema,
...
function bookingErrorMessage(code?: string) {
  ...
  if (code?.includes("GUEST_ACTIVE_LIMIT")) {
    return "Este telefone já possui 3 reservas futuras ativas. Entre em contato com a barbearia para revisar a agenda.";
  }

  if (code?.includes("INVALID_GUEST")) {
    return "Revise nome, telefone e e-mail antes de confirmar.";
  }
...
function parseBooking(formData: FormData) {
  return criarAgendamentoSchema.safeParse({
...
function revalidateBookingPaths(
  ...
  updateTag(getAvailabilityCacheTag(barbershopId));
...
export async function completarCadastro(formData: FormData) {
  ...
  const { user } = await exigirUsuario(slug);
  ...
  const { error } = await supabase.rpc("save_my_customer", {
    target_barbershop_id: barbershop.id,
    customer_name: parsed.data.full_name,
    customer_phone: normalizePhone(parsed.data.phone),
...
export async function criarAgendamento(
  ...
  const { error } = await supabase.rpc("book_appointment", {
    target_barbershop_id: barbershop.id,
    target_barber_id: parsed.data.barber_id,
    target_service_id: parsed.data.service_id,
    target_start_at: parsed.data.start_at,
  ...
  revalidateBookingPaths(barbershop.id, barbershop.slug);
...
export async function criarAgendamentoComCadastro(
  formData: FormData,
) {
  const slug = formSlug(formData);
  const { user } = await exigirUsuario(slug);
  ...
  const parsed =
    criarAgendamentoConvidadoSchema.safeParse({
      service_id: formData.get("service_id"),
      barber_id: formData.get("barber_id"),
      start_at: formData.get("start_at"),
      client_notes: formData.get("client_notes") || undefined,
...
export async function criarAgendamentoConvidado(
  formData: FormData,
) {
  const barbershop = await requireBarbershop(formSlug(formData));
  const supabase = await createClient();
  ...
  const { data, error } = await supabase.rpc(
    "book_guest_appointment",
    {
      target_barbershop_id: barbershop.id,
      target_barber_id: parsed.data.barber_id,
      target_service_id: parsed.data.service_id,
      target_start_at: parsed.data.start_at,
      guest_name: parsed.data.full_name,
      guest_phone: normalizePhone(parsed.data.phone),
      guest_email: parsed.data.email || null,
      target_client_notes: parsed.data.client_notes || null,
    },
  );
  ...
  return {
    ok: true,
    mensagem: reference
      ? `Reserva registrada com sucesso. Código: ${reference}. Guarde este código e o telefone informado.`
...
export async function cancelarAgendamento(formData: FormData) {
  ...
  const parsed = cancelarAgendamentoSchema.safeParse({
    ...
    cancellation_reason:
      formData.get("cancellation_reason") || undefined,
  });
  ...
  const { error } = await supabase.rpc(
    "cancel_my_appointment",
    {
      ...
      reason: parsed.data.cancellation_reason || null,
    },
  );

  if (error?.message.includes("CANCELLATION_LIMIT")) {
    return {
      ok: false,
      mensagem:
...
export async function registrarPagamentoPresencial(
  ...
  const { user, barbershop } =
    await requireBarbershopManager(formSlug(formData));
