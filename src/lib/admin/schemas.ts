...
export const barbeiroAdminSchema = z
  .object({
    ...
    phone: z.preprocess(
      ...
      z
        .string()
        .regex(telefoneOpcionalRegex, "Informe um telefone válido com DDD.")
        .nullable(),
    ),
    ...
    start_time: z
      .string()
      .regex(horaRegex, "Informe um horário inicial válido."),
    end_time: z.string().regex(horaRegex, "Informe um horário final válido."),
    break_start: z.preprocess(
      limparHoraOpcional,
      z
        .string()
        .regex(horaRegex, "Informe um início de pausa válido.")
        .nullable(),
    ),
    break_end: z.preprocess(
      limparHoraOpcional,
      z.string().regex(horaRegex, "Informe um fim de pausa válido.").nullable(),
    ),
    is_active: z.boolean().default(true),
  ...
  .superRefine((dados, ctx) => {
    ...
    const informouApenasUmaPausa =
      Boolean(dados.break_start) !== Boolean(dados.break_end);
    if (informouApenasUmaPausa) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["break_start"],
        message:
          "Informe início e fim da pausa ou deixe os dois campos vazios.",
      });
    }
